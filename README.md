...
```python
from monoprop import MajoranaPropagator, Gate, Term
...
gate = Gate(0, (Term((4, 5), -1.0),))  # param 0; gen_coeff folds M_γ's i prefactor

mp = MajoranaPropagator(observable, initial_state=[], cutoff=16)
mp.propagate([gate], [0.5])  # one angle value per parameter index
print(mp.evolved_operator_dict())  # the gate splits the monomial into two terms
```

The same simulator also accepts qubit (Pauli) operators via `PauliPropagator`, which
maps Paulis into the Majorana basis for you. Here we back-propagate `Z ⊗ Z` through one
`exp(-i θ/2 · X_0)` rotation:
...
gate = PauliGate(0, (0,), PauliOperator(["X"], [1.0]))  # exp(-i θ/2 · X_0)

mp = PauliPropagator(observable, initial_state=[], cutoff=16)
mp.propagate([gate], [0.5])  # one angle value per parameter index
# WARNING: keys are Majorana indices, not Pauli strings
print(mp.evolved_operator_dict())  # the gate splits Z ⊗ Z into two terms
```
