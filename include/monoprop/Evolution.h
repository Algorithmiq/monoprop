 ...
 */
// Recompute-routed (cos via callback): every layer's cosine scaling is performed by
// cos_scale(layer_index, …) — no layer stores its cos bitmap, so the callback (recompute fold /
// transient or filtered word list) is mandatory. Used by the in-build contraction and replay.
