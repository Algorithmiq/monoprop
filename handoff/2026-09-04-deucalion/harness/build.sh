...
# monoprop#314 made this the SINGLE switch for MPI. mpi4py is no longer in
# build-system.requires; a [[tool.scikit-build.overrides]] block keyed on
# `if.env.monoprop_ENABLE_MPI` injects it into build.requires AND sets the cmake define.
# An override's if. clause matches the ENVIRONMENT only, so the config-settings form below
# cannot trigger it: the define arrives, mpi4py does not, and the build dies in
# src/monoprop/bindings/CMakeLists.txt with "MPI is enabled but mpi4py is not importable".
# mpi4py lands in the project venv either way, which is why the failure reads as a puzzle --
# the missing copy is the one in scikit-build-core's isolated BUILD environment.
export monoprop_ENABLE_MPI=ON
