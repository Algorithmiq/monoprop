...
# REFUSES rather than defaults. VENV hangs off this, so a silent default hands the
# caller some other tree's _core.so and a venv with no monoprop_bench_tools -- every
# rank then dies inside benches/conftest.py and reports rc=4 per rep, which reads as
# measurements rather than as zero. That cost 20 runs across two 32-node allocations.
if [ -z "${MONOPROP_SRC:-}" ]; then
    echo "env.sh: export MONOPROP_SRC to the worktree you mean before sourcing this." >&2
    echo "  e.g. export MONOPROP_SRC=$PROJ/aaron/worktrees/pr5v2" >&2
    echo "  Trees: $(ls "$PROJ/aaron/worktrees" 2>/dev/null | tr '\n' ' ')" >&2
    unset _here
    return 1
fi
export MONOPROP_SRC
export MONOPROP_RUNS="$PROJ/aaron/runs"
...
_arch="$(uname -m)"
case "$_arch" in
    x86_64) _uv_root="$PROJ/aaron/tools/uv" _uv_tag="" ;;
    aarch64) _uv_root="$PROJ/aaron/tools/uv-arm" _uv_tag="-arm" ;;
    *) _uv_root="" _uv_tag="" ;;
esac
...
export UV_CACHE_DIR="$PROJ/aaron/caches/uv$_uv_tag"
export UV_PYTHON_INSTALL_DIR="$PROJ/aaron/caches/uv-python$_uv_tag"
...
export CPM_SOURCE_CACHE="$PROJ/aaron/caches/cpm"
