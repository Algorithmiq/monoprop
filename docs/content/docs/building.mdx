...
- Use `just test-profile` for the `monoprop_ENABLE_PROFILE` configuration, which
  compiles the `monoprop_PROFILE` instrument described in
  [Parallelism and distribution](/features/parallelism).
