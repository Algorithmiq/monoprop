...
it explicitly: `1024` with architecture flags on, `96` without them. The crossover
moves because dense monomials cost one pass per storage word while sparse rows are
flat in the width, so a target without a vector `popcount` reaches the crossover an
order of magnitude sooner. Both values are measured, not guessed; the number is
build-time because the ISA is.
