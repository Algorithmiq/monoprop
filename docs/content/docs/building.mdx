...
from it rather than being an option of its own: `768` storage modes when the build
actually emits architecture flags, `256` when it does not — which covers the published
wheels and any `Debug` build, where the flags are suppressed. Dense monomials cost one
pass per storage word while sparse rows are flat in the width, so a target without a
vector `popcount` reaches the crossover sooner. The number is build-time because the
ISA is, and there is no crossover you could pin that would be right independently of
the flags you compiled with.
