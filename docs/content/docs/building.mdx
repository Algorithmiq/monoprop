...
The QA workflow uses two explicit GCC sanitizer profiles, configured with
...
SKBUILD_CMAKE_DEFINE="CMAKE_CXX_COMPILER=g++-14;monoprop_GCC_SANITIZER=tsan" \
uv sync --group workspace-test --all-extras -v
ctest --test-dir build/editable/Tsan --output-on-failure
```
