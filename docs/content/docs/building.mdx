...
| `monoprop_FAT_MTUNE` | `skylake` | The `-mtune` value every fat-binary tier is scheduled for. |
...
it explicitly: `768` with architecture flags on, `256` without them — and `256` for a
fat binary, pinned across every tier so that a wide run's term order cannot depend on
the host CPU. The crossover
...
- Use `just build-fat`, `just test-variants` and `just diff-baseline-variants` for the
  multi-ISA build described in [The Fat Binary](/fat-binary).
