...
gate $U_j$ sends each monomial $M_\nu$ into one of three cases,
...
accumulate over a long sequence of gates. Conversely, a term whose coefficient
exceeds an upper threshold can be kept even when it is beyond the length cutoff.
