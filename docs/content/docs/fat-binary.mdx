...
an ifunc resolver picking a clone per call. It was rejected, and for a specific reason rather than a
stylistic one.

The vectorization in the table above lands overwhelmingly in *headers* — `Bitset.h`, the algebra's
cutoff accumulation, the row-store accessors — which are inlined into their callers, and which the
scan instantiates a dozen times over across the basis, row-backend and word-width seams. A
per-function dispatch boundary is exactly an inlining boundary: multiversioning the kernels would
suppress the inlining the win depends on, in order to deliver the win. There is no small set of
standalone hot functions to clone.
