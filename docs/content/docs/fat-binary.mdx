...
The duplication *is* reducible, though not that way: 91% of the vectorized loops and 51% of the
engine's `.text` are in `MonomialPropagator.cpp` alone, so tiering that one translation unit and
sharing a single baseline copy of the other nine would cut the shipped payload by roughly 40%. The
price is a real dispatch seam at `build_evolve_result_` rather than at the module boundary.
