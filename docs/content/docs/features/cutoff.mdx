...
`length` (counts Majorana operators $m_j$) and `support` (counts the distinct
modes touched). This is the natural choice for fermionic problems.
...
Pauli operators and gates directly and the `cutoff` parameter bounds the Pauli
weight. Reach for it whenever your problem is naturally expressed in qubits.
