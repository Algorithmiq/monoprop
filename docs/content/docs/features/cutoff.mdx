...
complete pair $m_{2j-1}m_{2j}$ on some mode, or in qubit space is a product of Pauli-$Z$ operators, is *always kept*, regardless of
...
sim.upper_atol = 1e-3
```
Note that choosing a very low value for `upper_atol` results in increased number of the
