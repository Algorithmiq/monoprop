...
Schrödinger picture the state is evolved independently, so you can re-weight the
observable and read it off against the same evolved state; in the Heisenberg
picture the same graph can be reused with different initial operators.
