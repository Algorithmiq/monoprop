...
In C++ the choice is a `PictureSpec` — `Heisenberg{}` or `Schrodinger{state_cutoff}` — so only a
Schrödinger run carries a state cutoff. The Python constructors keep the `schrodinger_cutoff`
spelling and resolve it at the binding boundary.
