...
on fermionic circuits when `schrodinger_cutoff = cutoff + 2` and when the gates are generated
by length-4 Majorana monomials (see Appendix of
[@chakraborty2026scalablequantumcircuitgeneration]).
