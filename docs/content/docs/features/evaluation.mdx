...
The parameter axis follows that equivalent circuit, not the order the calls were made in,
so the incremental build and the one-call build of the composition are interchangeable —
same [parameter_mapping][monoprop.monomial_propagator.MonomialPropagator.parameter_mapping],
same expectation value and gradient anywhere. In Heisenberg each extension therefore takes
the low indices and lifts the ones already in the graph by `circuit.n_parameters`.
