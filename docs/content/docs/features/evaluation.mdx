...
[simulation mode](/concepts/simulation_modes), because the Heisenberg picture consumes each
...
The parameter axis follows the same order. In Heisenberg each extension takes the low indices
and lifts the ones already in the graph by `circuit.n_parameters`.

Gate *arrival* order is not renumbered. The two builds number their
