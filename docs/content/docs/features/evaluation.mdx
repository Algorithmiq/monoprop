...
mutation that fails part-way through a *partitioned* propagator retires the propagator itself, so
neither path answers again — see [Parallelism and distribution](/features/parallelism).
...
| a rejected `update_initial_operator` | raises: a partitioned propagator applies the re-weight one partition at a time, so a term only one of them holds is rejected with its siblings already re-weighted |
