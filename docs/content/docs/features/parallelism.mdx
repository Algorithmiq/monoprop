...
rather than enforced at runtime: a `COMMROUTE` line on stderr reporting the measured $\rho$
against the $d$ the geometry needs. The diagnostic reads the $d$ routing bits of each shift,
so its $\rho$ is bounded by $d$ — at $R = 128$ the 60-site Hubbard's 416 distinct shifts
report the full $\rho = d = 7$. The basis itself is drawn full rank, so a short $\rho$ always
means the generator set, never the seed.
...
for the `splitmix` hash of $M$, the partition is $q \bmod S$ and the rank is $h_d(M)$ with
$d = \log_2 R$ — every rank bit, so the fanout is 1:
...
XOR structure to route by, so [MajoranaPropagator][] and [PauliPropagator][] raise at
construction rather than route silently. Launching a non-power-of-two rank count therefore
needs `monoprop_ROUTING=splitmix`. $R = 1$ is a power of two, takes no rank bit, and so is
the dense case already. Linear routing also needs $\log_2 R \le 2N$ — a rank index cannot
carry more bits than the monomial has — which is likewise raised rather than routed.
...
| `monoprop_ROUTE_SEED` | `6768574230969066775` | Decimal `uint64` from which every rank derives the same basis $\{v_i\}$ with no communication. The seed is redrawn until the basis is full rank, so a seed can never be the reason ranks sit idle. The same value must reach every rank: a mismatch in either of these variables, or in the partition count, is caught by one allreduce the first time a communicator is used and raised, because under linear routing it deadlocks the exchange instead of corrupting it. |
