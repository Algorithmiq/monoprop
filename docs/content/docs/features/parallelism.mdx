...
partitions rather than by problem size, and it is not a build option. Crossing it
raises a `RuntimeError` naming the ceiling instead of wrapping silently; the fix is
more partitions or more MPI ranks — both divide the terms per partition — or a
