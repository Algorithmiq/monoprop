...
hash table over the operator: each gate builds a small table over its anticommuting rows,
keyed by the mixed fingerprint with every match confirmed against the row's positions, and
discards it. The operator store holds positions and coefficients and nothing else.
