...
| Variable | Default | Meaning |
| --- | --- | --- |
| `monoprop_ROUTING` | `linear` | `splitmix` (the literal string) forces $d = 0$, the dense all-to-all. Any other value, including unset, selects linear routing with $d$ as large as the geometry allows. |
| `monoprop_ROUTE_LINEAR_BITS` | unset | An explicit $d$, clamped to $[0, \log_2 R]$. Takes precedence over `monoprop_ROUTING`; a value that is zero, negative or unparseable means $d = 0$. |
