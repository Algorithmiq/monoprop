...
or not the process will ever send a message. On one login node with eight host adapters that
...
`monoprop_TEST_EXCLUDE_MPI_FABRIC=ON`, which skips fabric init for the per-case tests. What
that is worth scales with how many adapters the machine has, so read it per case rather than
as a suite wall: on a Deucalion `dev-x86` compute node the exclusion takes `ctest -L serial`
from 2.03 s per case to 0.61 s (435.31 s over 214 cases against 126.03 s over 208), a median
of 3.42x over the cases the two runs share.
