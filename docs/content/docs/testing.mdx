...
or not the process will ever send a message. It looks like slow tests, but it is wall time
with no CPU behind it. So MPI builds default to `monoprop_TEST_EXCLUDE_MPI_FABRIC=ON`, which
skips fabric init for the per-case tests; the saving scales with how many adapters the
machine has, and was measured at 3.4x per case on a well-connected compute node.
