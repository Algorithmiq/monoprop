...
`monoprop_WIDE_TERM_INDEX` branches, so it has its own recipe. The environment
variable is a scikit-build source-build switch, just like `monoprop_ENABLE_MPI`.
The overrides merge their CMake definitions, so setting both variables enables
the combined MPI and wide-index build:

```bash
just test-wide   # rebuilds via uv sync with monoprop_WIDE_TERM_INDEX=ON, then runs CTest
monoprop_ENABLE_MPI=ON monoprop_WIDE_TERM_INDEX=ON uv sync --all-extras
```

CI gives every compiler, MPI, and term-index combination a distinct setup-uv
cache suffix, because uv's cache key does not include build environment variables.
