...
Release builds use `-O1` for the test sources while retaining `-O3` for the
library.
...
CTest runs each Catch2 case as its own process, so an MPI build's `MPI_Init` probes every
...
CTest registers every Catch2 case individually as a `serial` variant. When the
...
C++ tests live in `cpp/tests/` and use [Catch2 v3](https://github.com/catchorg/Catch2). They are registered automatically via CMake: new `*.cpp` files in `cpp/tests/` are picked up on the next configure, so no source-list edit is needed.
...
Use the `ExampleDataFix` fixture class from `TestUtilities.h` to load the same msgpack data as Python tests, and Catch2 generators to parametrize over it:
...
TEST_CASE_METHOD(ExampleDataFix, "my_new_test") {
