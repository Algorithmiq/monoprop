...
The option table, and why it is a build option rather than an environment
variable, are in [Building from source](/building). Leave it off for timing runs —
it reinstates the per-exchange collective the derivation removes.
