...
`cpp/tests/wide_system_tests.cpp` relabel the 12-mode LiH fixture into a 90-mode
system, which stores at 96 modes — three words per monomial, an active window six
modes short of its storage, and mode lanes past the first codes word. A released
wheel selects sparse rows there on its own; a build with `monoprop_ENABLE_ARCH_FLAGS`
on gets there through `monoprop_ROW_STORE=sparse`, and both suites assert they did,
so the sparse variant cannot silently be a second dense run. See
`tests/data/README.md` on why a wide case is derived rather than checked in.
