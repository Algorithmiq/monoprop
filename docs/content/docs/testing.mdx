...
suite that propagates monomials whose words live on the heap. A released wheel and a default source build
both select sparse rows at that width on their own; a build with
`monoprop_ARCH_MARCH=native` (crossover `768`) gets there through
`monoprop_ROW_STORE=sparse`, and both suites assert they did, so the sparse variant
cannot silently be a second dense run. See `tests/data/README.md` on why a wide case is derived rather than checked
in.
