...
Run it whenever you change a stored layout. Continuous integration compiles the
wide build in a single matrix cell and every other build — local, packaged, and
on the clusters — is narrow, so a wide-only regression has exactly one place left
to surface. For the same reason, an assertion about a type's size or alignment
should state the **rule** rather than a byte count, and should be a
`static_assert` so that it fails the compile instead of waiting to be run: a guard
written as `sizeof(TermIndex) != sizeof(uint32_t) || <the real check>` switches
itself off in the only configuration that could have failed it. The two
assertions beside `CrossRankOccupiedSlot` in
`cpp/monoprop/detail/graph_encoding/MPGraphEncodingTypes.h` are the worked
example: one expression that reads 12 bytes narrow and 24 wide.
...
The distributed exchange derives each layer's recv layout from its send layout
instead of resolving one over the wire. To audit that derivation against a real
all-to-all on every exchange, rebuild with the check compiled in and run the MPI
suite:

```bash
SKBUILD_CMAKE_ARGS="-Dmonoprop_ENABLE_MPI=ON;-Dmonoprop_CHECK_EXCHANGE_SYMMETRY=ON" \
    uv sync --all-extras --reinstall-package monoprop --no-cache
mpiexec -n 4 build/editable/Release/bin/monoprop_unit_tests.x
```

It is a build option rather than an environment variable because the audit is a
collective; the option table is in [Building from source](/building). Leave it off
for timing runs — it reinstates the per-exchange collective the derivation removes.
