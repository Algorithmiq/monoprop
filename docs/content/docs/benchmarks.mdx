...
One build note: the Julia engine here
...
| Option | Meaning | Default |
|---|---|---|
| `--model-rounds` | rounds per model; each rebuilds the model first | 1 |

The default of one round records the memory and term-count statistics, which are deterministic, but
gives no spread for the timing. Pass `--model-rounds 5` or more whenever a timing *difference* is the
point of the run — a single sample on these models can sit well off the median, and comparing two
one-sample runs will read noise as signal.
...
The **Configuration** table names the row backend the run actually used, as `asked → ran`. Both halves
are needed: `monoprop_ROW_STORE` records what was requested, but its default of `auto` leaves the
choice to the storage width against a build-time crossover, so the setting alone does not identify the
backend — and the two backends differ in footprint and in the order they accumulate a term sum. A run
whose propagators disagree (they can only differ by width) reads `mixed`.

The random benchmarks are reported per physical picture, since their size comes from the CLI and is
shared by every label. The fixed models get a **Fixed models** section instead, one table each, because
each model carries its own mode count: term count, time, **cost per term**, peak and resting RSS, and
monoprop's own operator accounting with its **bytes per term**. The two per-term figures are the ones to
compare across widths — the models grow their term count with their mode count, so the totals beside
them are not comparable on their own.
