...
| Workflow | Runner | Testbed | Label |
|---|---|---|---|
| `bench_main.yml` | GitHub-hosted `ubuntu-26.04` | `ubuntu-26.04` | `ci-linux` |
| `bench_bare_metal.yml` | [RunsOn](https://runs-on.com/) EC2 node | `aws-c7i-4xlarge` | `ci-bare-metal` |
...
thresholds need setting once enough points exist to see the noise. Problem sizes
are shared with the GitHub-hosted testbed until they are calibrated on the real
node; `bench-ci` forwards extra arguments to `pytest` for that, so widening the
profile for one machine does not need a second recipe:

```bash
just bench-ci ci-bare-metal -m ""   # include the slow fixed models
```
