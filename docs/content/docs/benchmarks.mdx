...
`MajoranaPropagation.jl` at 28 — since they do not peak at the same count. Every record carries
its own `num_threads`, `busy_cores`, `host` and `library_version`, so a redrawn figure never
