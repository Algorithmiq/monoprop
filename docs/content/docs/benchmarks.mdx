...
The default sizes above fit a laptop and a CI runner; the sizes the library is used at do not.
`benches/LADDER.md` is the set at those sizes, run through the ordinary
`just bench LABEL <flags>` route (or that same pytest line under `srun` for multi-rank): four
groups, each a table of the exact flags and `-k` selector that produce it. L1 is one thread at
~10M terms; L2 is one node at ~1B; L3 is the same problems on four nodes at 8 ranks/node x 16
partitions; L4 is the strong and weak scaling ladders of hubbard `propagate`. The document also
gives the sbatch shapes for each group and the pinning traps that silently unpin a run.

Nothing runs these for you, and no benchmark gates a pull request. Running the group a change
could plausibly move, and putting the numbers in the pull request, is the author's job.

### Scaling

Node scaling of Hubbard `propagate`, 1 to 64 nodes (128 to
8192 cores) at 8 MPI ranks per node × 16 partitions, so `R = 8N` and `P = 128N` with one core per
partition. Measured 2026-08-27 on AMD EPYC 7742 nodes with 242.0 GiB usable each; 38 points, five
reps apiece (ten on the smallest weak ladder), every rep gate-checked on binary identity,
environment and term count before its time was kept. `benches/LADDER.md`'s L4 group reproduces
these points; the medians below are what a run of it is a diff against.

The measuring binary was `perf/linear-routing-on-wire` @ `c2554554` (`_core.so` md5
`1157a5e2b421fb8bd18fc6d16fa39778`), whose default is GF(2)-linear on-wire routing. That is the
branch's default, **not** `main`'s: the figures are internally consistent and traceable, but they
are not a measurement of `main`.

Problem size is `--hubbard-lower-atol` at a fixed cutoff of 10, which is saturated there, so all
growth comes from the tolerance and every point stays inside the same operator family. The eleven
...
Some points are absent because the problem does not fit in that many nodes' memory, not because
...
Peak resident memory over all 38 points, spanning 25 to 3063 Mterms per node:

```
GiB/node = 3.39 + 0.0634 × Mterms/node
```

The law is **specific to hubbard `propagate` at 8 ranks x 16 partitions** and does not transfer:
the random model at ~949M terms measures 0.100 GiB/Mterm propagating, 0.115 building the graph
and 0.115 evaluating energy and gradient, against this slope's 0.063. Both terms of the fit are
real. The slope is the per-term cost, about 68 bytes per term. The constant is a
genuine per-node floor that a model through the origin cannot reproduce — the smallest point
measures 4.1 GiB/node at 25 Mterms/node. Every point falls within −2.9 to +4.5 GiB/node of that
...
strength of one point. The largest cell measured 201.3 GiB/node against a 242.0 GiB grant.

Memory partitions across nodes for hubbard and pauli and **does not** for the random model, whose
per-node peak barely moves on any operation. The cause is a per-process floor. For the random
`build_graph` row of L2 against L3, `opmembase` is the resident floor when the timed call starts
and `opmemdelta` is the call's own cost:

| per rank | 1 node, R=1 | 4 nodes, R=32 |
| --- | ---: | ---: |
| floor (`opmembase`) | 13.04 GiB | 9.45 GiB |
| the call itself (`opmemdelta`) | 96.25 GiB | 3.59 GiB |

Both columns reconcile to the measured peak exactly: `13.0 + 96.3 = 109.3` GiB/node, and
`8 x 9.45 + 8 x 3.59 = 104.2`. The operation's own memory partitions almost perfectly -- 27x less
per rank for 32x the ranks -- while the floor is a **~9.5 GiB per-process constant**, so eight
ranks on a node pay it eight times and 75.6 of that row's 104.2 GiB/node is floor before the graph
build allocates anything. It is not per-partition (128 partitions carry 13.04 GiB, 16 carry 9.45)
and it is not the operator: every field of `opmembreak` partitions cleanly, its rank-summed total
moving only 88.0 to 86.0 GiB from 1 rank to 32. What the 9.5 GiB consists of is **not attributed**
by any recorded field; allocator-retained construction memory, fabric buffers and per-process
thread state are the unmeasured candidates. The consequence is the useful part: **more ranks per
node buys speed and costs a floor**, paid per process, so it hurts most where the operation's own
cost is smallest -- worst on the 19M-term Schrodinger rows, where four nodes cost more per node
than one.
...
**No benchmark workflow gates a pull request.** Both track the main branch, and a threshold
over a series that spans runners is a report, not a verdict. Measuring a change is the author's
job -- see "The ladder" above. Every run uploads `bmf.json` and the raw result JSON as an artifact,
so a failed Bencher upload does not take the measurement with it.
