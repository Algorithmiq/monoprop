...
`monoprop` has the lowest measured working-memory value in both comparisons. All results were
obtained on [Leonardo (CINECA)](https://docs.hpc.cineca.it/hpc/leonardo.html); the experimental
conditions are specified under [Methods](#methods).
...
<Caption>
  Solid lines denote peak RSS. Faint lines denote implementation-specific memory estimates and
