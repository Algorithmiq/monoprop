...
step tried, 3.5M, reaches 35.49 GiB/node, past the 32 GiB the instance has. A two-point fit over
1.5M and 2.5M predicted 31.1 GiB there and under-called it by 3.9, so this axis is read off
measured points rather than interpolated.
...
The workflow is inert until a maintainer sets the `BENCH_BARE_METAL` repository variable to
`true`; without a RunsOn stack behind the label the job would queue until it timed out, once per
commit. There is no shared-runner series: a GitHub-hosted runner has about two physical cores, so
L2a would be 1 × 2 and L2b 2 × 1 — restatements of L1 through a noise floor no threshold survives.
