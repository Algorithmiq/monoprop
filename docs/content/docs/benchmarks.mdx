...
We compare `monoprop` with open-source propagation engines on Majorana- and Pauli-basis
quantum-simulation workloads. Full settings, caveats, and reproduction instructions follow the
plots.

### Majorana propagation

We propagate an observable through a Trotterized 1D Fermi-Hubbard circuit for 20, 40, and 60
spinful sites. `monoprop` is 22–142× faster than `MajoranaPropagation.jl` over the tested grid;
final overlaps agree within `1.8e-6`.
...
Number of terms, final overlap, runtime, and memory versus circuit depth.

### Pauli propagation

We propagate a `ZZ` observable through a Trotterized 12×12 transverse-field Ising model.
`monoprop` is the fastest CPU engine as the operator grows and tracks GPU-accelerated
`cuPauliProp`.

![Benchmark results for Pauli Propagation engines: Runtime and Memory per Trotter step](/benchmarks/pauli_results.png)

Runtime and process peak memory per Trotter step for five propagation engines.

## Benchmark details and reproduction

The benchmark code, locked dependencies, configurations, and committed results are in
[`packages/bench-third-party`](https://github.com/Algorithmiq/monoprop/tree/main/packages/bench-third-party).
That directory is a standalone `uv` project with its own lockfile because it pins CUDA-specific
wheels and a narrower Python range than `monoprop` itself.

The comparisons use each engine's fastest usable configuration. They are wall-clock measurements,
not equal-core or equal-hardware comparisons, and should be read as results for these workloads
rather than a general package ranking.

### Majorana propagation details
...
Runtime on linear axes; solid lines are `monoprop` and dashed lines are
