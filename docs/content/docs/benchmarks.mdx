...
- `native_memory`: where an engine accounts for itself, its own figure (monoprop's C++
  operator-memory accounting, cuPauliProp's cupy device memory pool, `PauliPropagation.jl`'s
  `Base.summarysize` of the Pauli sum). These are reference values only: they measure
  different things and must not be compared across engines. `QuEra ppvm` and `Qiskit
  pauli-prop` expose no such accounting and so report none.
