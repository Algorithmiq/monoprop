...
- `native_memory`: where an engine accounts for itself, its own figure (monoprop's C++
  ...
  only: they measure different things and must not be compared across engines. monoprop's
  own total also steps up when a container gains a field that counted it nowhere. `QuEra ppvm`
