...
We compare `monoprop` with open-source Majorana and Pauli propagation engines on two
quantum-simulation workloads. The benchmark code, locked dependencies, configurations, and
committed results are in [`packages/bench-third-party`](https://github.com/Algorithmiq/monoprop/tree/main/packages/bench-third-party).

### Results at a glance

| Workload | Comparison | Main result | Cross-check / context |
| --- | --- | --- | --- |
| 1D Fermi-Hubbard, 20–60 spinful sites | `MajoranaPropagation.jl` | `monoprop` is 22–142× faster over the tested grid | Final overlaps agree within `1.8e-6` |
| 2D TFIM, 12×12 lattice | Four Pauli engines | `monoprop` is the fastest CPU engine as the operator grows and tracks GPU-accelerated `cuPauliProp` | Per-step expectation values and term counts are recorded |
| 2D TFIM scaling, 36–324 qubits | Four Pauli engines | 16–105× over `PauliPropagation.jl` and 4.4–15× over GPU `cuPauliProp` | Same model; backend caveats are reported below |

These are wall-clock comparisons of each engine's fastest usable configuration, not equal-core or
equal-hardware comparisons. They measure the workloads and truncation regimes described below;
they are not a general ranking over every problem. Threading, timeouts, storage semantics, and
memory-measurement caveats are reported with the corresponding result.
The published comparisons report individual measurements rather than runtime distributions, so
the ratios should be treated as workload-specific measurements rather than uncertainty bounds.
...
We propagate an observable through a Trotterized 1D Fermi-Hubbard circuit for 20, 40, and 60
spinful sites, tracking runtime, final overlap, operator size, and process memory.
...
Final overlaps agree within `1.8e-6`, providing a cross-check on the observable computed by the
two engines. `monoprop` retains 1.44–1.50× more terms at the same `atol` and cutoff.

`monoprop` is 22–142× faster over the tested grid: 21.9× at 20 sites / 10 layers and
141.6× at 60 sites / 18 layers (4.2 s versus 592 s). Memory is comparable at low depth and
diverges in `monoprop`'s favour as the operator grows (2834 MB versus 3390 MB at 60 sites / 18
layers).

The engines use their respective fastest thread counts: `monoprop` uses 56 threads and
`MajoranaPropagation.jl` 28. Normalizing by the measured busy-core count leaves an 8.5–52×
difference. This indicates that thread count alone does not explain the wall-clock gap, but it
does not isolate algorithmic effects from vectorization, memory layout, or parallel efficiency.
...
Each engine runs at the thread count it is fastest at — `monoprop` at 56 threads and
...
We propagate a `ZZ` observable through a Trotterized 12×12 transverse-field Ising model (TFIM):

- `hx=1.0`, `hz=1.0`, and `j=1.5`
- Trotter step `dt=0.05`
- steps 0–55, sampled every two steps
- coefficient tolerance `lower_atol=1e-6`
- no Pauli-weight cutoff

The complete configuration is in [`settings.json`](https://github.com/Algorithmiq/monoprop/blob/main/packages/bench-third-party/pauli_prop/settings.json).
...
other Pauli propagation engines.

#### Results

As the operator grows, `monoprop` outperforms the other CPU-only engines in wall-clock time and
process peak RSS, while tracking GPU-accelerated `cuPauliProp`. The first step is excluded from
timing because it includes JIT compilation and first-touch allocation.

The result file also records per-step expectation values and term counts. Process peak RSS is
measured consistently across engines; engine-native memory figures are retained as reference
values but are not compared because they measure different quantities.
...
The benchmark above fixes the lattice at 12×12 and walks the Trotter steps. The second Pauli
benchmark does the opposite: it holds the circuit depth and the truncation fixed and grows the
lattice, from 6×6 (36 qubits) to 18×18 (324 qubits). Every engine runs the identical model at
...
The same saturation shows up in memory: `monoprop`'s footprint flattens near 690 MB from 12×12
