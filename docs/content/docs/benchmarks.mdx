...
The figure compares wall-clock time per propagation step on a logarithmic scale. At the final
Pauli step, the runtime ratios relative to `monoprop` are 267-325× for the other CPU
implementations and 7.8× for the GPU implementation. Over the complete Majorana evolution, the
runtime ratio for `MajoranaPropagation.jl` is 27.7×. `monoprop` also has the lowest measured
working-memory value in both comparisons. All results were obtained on
...
operator resides in GPU memory; consequently, the table reports its peak device allocation rather
than its 552 MB host RSS.

![Runtime and peak RSS per Trotter step for five Pauli propagation engines](/benchmarks/pauli_results.png)

Solid lines denote peak RSS. Faint lines denote implementation-specific memory estimates and are
not comparable across implementations.
...
million terms above 100 qubits. Beyond this point, the experiment primarily measures the cost per
retained term. Across the full range, `monoprop` runtime increases from 6.35 s to 14.1 s (2.2×),
while the qubit count increases 9×.

The 300 s limit excludes `QuEra ppvm` and `Qiskit pauli-prop` at all lattice sizes, and excludes
`PauliPropagation.jl` above 10×10. The endpoints of these curves therefore indicate timeouts, not
...
`cuPauliProp`. Across the full range, `monoprop` increases from 5045 MB to 7835 MB (1.55×), while
`cuPauliProp` increases from 7.6 GB to 34.4 GB. At 324 qubits, the latter is 4.5× the
...
`PauliPropagation.jl` over its completed points, and from 4.2× to 20× for `cuPauliProp` over the
full range.
...
The separate linear-scale plot preserves the absolute runtime ratio. The annotation gives the
upper bound of the `monoprop` curve, which otherwise lies close to the horizontal axis.
...
- **The CPU and GPU measurements use different node types.** Each scaling curve is obtained on a
  single node, and the plotting scripts reject curves whose records contain multiple hosts. The
...
these fields alongside derived metrics. The measurements use `monoprop 0.8.1.dev61+gcb9a033aa`,
Julia 1.12.6, and `PauliPropagation.jl` 0.8.2.

### How memory is measured

CPU memory denotes **peak resident set size (RSS)**, obtained from the Linux kernel's `VmHWM`
counter. This definition is independent of the implementation language. For `cuPauliProp`, whose
operator resides on the GPU, working memory instead denotes peak device allocation.

- **The measurement is exact rather than sampled.** The kernel updates `VmHWM` whenever RSS
...
across ranks. Memory is the **peak of the summed concurrent PSS**, reconstructed from per-rank
timelines. This procedure excludes nonconcurrent per-rank peaks and counts shared pages once.
