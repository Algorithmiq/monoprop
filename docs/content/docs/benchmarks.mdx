...
![Runtime for the 1D Hubbard model in MajoranaPropagation.jl vs monoprop: seconds vs circuit layers, log scale](/benchmarks/majorana_runtime.png)

Runtime on its own, log scale. Solid lines are `monoprop`, dashed are
`MajoranaPropagation.jl`, and colour is the system size. The two engines separate into distinct
bands: on the shared linear axis of the combined figure above, `monoprop`'s curves are pressed
...
Each engine runs at its own measured thread optimum — `monoprop` at 56 threads,
`MajoranaPropagation.jl` at 28 — because they do not peak at the same count: across the sweep
`monoprop` holds 95-97% parallel efficiency out to 56 threads (11.9× over serial) while
`MajoranaPropagation.jl` plateaus near 71% and stops gaining past 28 (4.1× over serial). Both
regress beyond their optimum. Every record carries its own `num_threads`, `busy_cores`, `host`
and `library_version`, so a redrawn figure never misreports what produced it.
...
- `busy_cores`: `cpu_seconds / runtime_seconds` — how many cores the run actually kept busy.
  A value near 1 means the run was serial whatever `num_threads` claimed, so this is what makes
  a cross-library comparison interpretable rather than a core-count artifact.
...
grid, and `majorana_runtime.png`, the runtime alone on a log scale. Runtime gets its own file
because it is the quantity cited on its own, and the one the shared linear axis serves worst.
...
- **The memory figure is not one quantity.** `monoprop` and `cuPauliProp` report their own
