...
without rebuilding the simulator or the graph. The keys are Majorana monomials
(tuples of indices); the values are the new coefficients, which must keep each
monomial Hermitian (real for length-4 terms, imaginary for length-2 — see the
Notation page):
...
without re-propagating. Moreover, for the Heisenberg picture, the same graph
can be reused with different initial operators, so you can evaluate many observables
