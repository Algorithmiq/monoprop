...
.. testcode::
   ...
   # A FermiCircuit carries into Majorana gates: get_majorana_sequence() maps the
   # generators through Jordan-Wigner, and gates_from_majorana_sequence() bundles them
   # into the Gate/Parameter objects the propagator consumes.
   sequence = circuit.get_majorana_sequence()
   gates, _ = gates_from_majorana_sequence(sequence)

   mp = MajoranaPropagator(observable, sequence.initial_state, cutoff=4)
...
.. testcode::
   ...
   qgates, _ = gates_from_qubit_circuit(circuit)
   parameters = [g.parameter for g in circuit.gates]

   # QubitPropagator maps the Pauli gates and observable through Jordan-Wigner, and
   # sets the matching basis_change automatically so a "support" cutoff counts the
   # qubits touched — the Pauli weight — rather than Majorana modes (see the Notation
   # and Cutoff pages).
   mp = QubitPropagator(
...
.. testcode::
   ...
   from monoprop import MajoranaPropagator, Gate, Term, Parameter
   ...
   # One rotation generated by i·m_3 m_4. Authored directly as a Gate: the generator's
   # i prefactor folds into the Term's real gen_coeff, and a Parameter handle carries
   # the angle (reuse one handle across gates to tie their angles).
   theta = Parameter("theta")
   gate = Gate(theta, (Term((3, 4), -1.0),))

   mp = MajoranaPropagator(observable, initial_state=[0, 1], cutoff=4)
   mp.propagate([gate], {theta: 0.3})
...
each of which is a Majorana rotation. A gate (:class:`~monoprop.Gate`) is described
by two pieces of information:

- its **terms** - the Majorana monomials :math:`M_\gamma` generating the rotation
  :math:`e^{-i\theta M_\gamma/2}`, each a :class:`~monoprop.Term` pairing the monomial
  with a real generator coefficient; and
- a **parameter** - a :class:`~monoprop.Parameter` *handle* for the angle
  :math:`\theta`. Sharing one handle across gates ties their angles together (weight
  tying); the parameter values themselves are supplied per evaluation, not baked into
  the gate.
