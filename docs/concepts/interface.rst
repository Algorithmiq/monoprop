...
.. testcode::
   ...
   # A FermiCircuit carries into Majorana gates: to_gates() maps the generators through
   # Jordan-Wigner and bundles them into the MajoranaGate objects the propagator consumes,
   # returning (gates, parameters, parameter_mapping).
   gates, parameters, _ = circuit.to_gates()

   mp = MajoranaPropagator(observable, circuit.initial_state, cutoff=4)
...
.. testcode::
   ...
   qgates, parameters, _ = circuit.to_gates()
...
.. testcode::
   ...
   from monoprop import MajoranaPropagator, MajoranaGate, Term
   ...
   # generator whose i prefactor folds into the Term's real gen_coeff. The angle driving
   # it is supplied per evaluation (which angle drives each gate is set separately by the
   # propagator's parameter_mapping argument).
   gate = MajoranaGate((Term((3, 4), -1.0),))

   mp = MajoranaPropagator(observable, initial_state=[0, 1], cutoff=4)
   mp.propagate([gate], [0.3])
...
Which variational angle :math:`\theta` drives each gate is given separately by the
propagator's ``parameter_mapping`` argument (default: one distinct angle per gate);
``parameter_mapping[i]`` is the angle index driving gate ``i``, and sharing one index across
gates ties their angles together (weight tying). The angle values themselves are supplied
per evaluation as a plain sequence of floats (a list or numpy array), not baked into the
gate.

Gates are then handed to the propagator via
...
values. This structure enables the use of monoprop in variational workflows.
