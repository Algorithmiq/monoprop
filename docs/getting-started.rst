...
.. doctest::
   ...
   >>> # prefactor folds into gen_coeff. The angle driving it is supplied at evaluation.
   >>> gate = MajoranaGate((Term((4, 5), -1.0),))
   >>> mbs = MajoranaPropagator(observable, initial_state=[], cutoff=16)
   >>> mbs.propagate([gate], [0.5])  # evaluate the gate angle θ = 0.5
...
.. doctest::
   ...
   >>> import numpy as np
   >>> from monoprop import PauliPropagator, PauliGate
   ...
   >>> mbs.propagate([gate], [0.5])  # PauliPropagator maps X_0 to Majoranas
