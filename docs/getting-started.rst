...
.. doctest::
   ...
   >>> # One Majorana rotation exp(-i θ/2 · M_γ) generated by M_γ = i m_4 m_5. A gate is a
   >>> # pure generator wrapping the MajoranaOperator it generates; the i prefactor folds
   >>> # into the real generator coefficient. A MajoranaCircuit pairs the gate with its angle.
   >>> gate = MajoranaGate(MajoranaOperator.from_dict({(4, 5): -1.0}))
   >>> circuit = MajoranaCircuit(gates=[gate], parameters=[0.5])  # gate angle θ = 0.5
...
.. doctest::
   ...
   >>> gate = PauliGate((0,), PauliOperator(["X"], [1.0], num_qubits=1))  # exp(-i θ/2 · X_0)
   >>> circuit = PauliCircuit(gates=[gate], parameters=[0.5], num_qubits=2)
