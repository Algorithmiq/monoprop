...
.. doctest::
   ...
   >>> # One Majorana rotation exp(-i θ/2 · M_γ) generated by M_γ = i m_4 m_5. Exp is the
   >>> # exponential of the MajoranaOperator it wraps (an operator carrying its mode count);
   >>> # the i prefactor folds into the real generator coefficient. A Circuit pairs it with θ.
   >>> gate = Exp(MajoranaOperator({(4, 5): -1.0}, num_modes=8))
