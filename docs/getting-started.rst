...
.. doctest::
   ...
   >>> # bundles the monomials it generates as Terms; the i prefactor folds into gen_coeff.
   >>> theta = Parameter("theta")
   >>> gate = Gate(theta, (Term((4, 5), -1.0),))
   >>> mbs = MajoranaPropagator(observable, initial_state=[], cutoff=16)
   >>> mbs.propagate([gate], {theta: 0.5})  # evaluate the gate angle θ = 0.5
...
.. doctest::
   ...
   >>> import numpy as np
   >>> from monoprop import QubitPropagator, QubitGate, Parameter
   >>> from monoprop.pauli_data import PauliOperator
   >>> observable = PauliOperator(["ZZ"], [1.0])
   >>> theta = Parameter("theta")
   >>> gate = QubitGate(theta, (0,), PauliOperator(["X"], [1.0]))  # exp(-i θ/2 · X_0)
   >>> mbs = QubitPropagator(observable, initial_state=[], cutoff=16)
   >>> mbs.propagate([gate], {theta: 0.5})  # QubitPropagator maps X_0 to Majoranas
