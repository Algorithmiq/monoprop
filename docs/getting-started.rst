...
.. doctest::
   ...
   ...     MajoranaPropagator, MajoranaExp, Circuit, MajoranaOperator
   ... )
   >>> observable = MajoranaOperator({(0, 1, 2, 4): 1.0}, 8)  # 8 = number of fermionic modes
   >>> # One Majorana rotation exp(-i θ/2 · M_γ) generated by M_γ = i m_4 m_5. MajoranaExp is
   >>> # the exponential of the MajoranaOperator it wraps; the i prefactor folds into the real
   >>> # generator coefficient. A Circuit pairs the gate with its angle.
   >>> gate = MajoranaExp(MajoranaOperator({(4, 5): -1.0}))
...
.. doctest::
   ...
   >>> gate = PauliExp(Pauli("X", 0))  # exp(-i θ/2 · X_0)
