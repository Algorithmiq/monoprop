...
// The protocol's per-row state for one gate (Engine.h has the rules), as four bitsets over row indices.
...
// partner is tracked) and `partner_rot` (that record's rot bit).
//
...
private:
    ...
    auto arrays_() -> std::array<std::vector<uint64_t> *, 4> { return {&rot_, &foll_, &received_, &partner_rot_}; }
    auto arrays_() const -> std::array<const std::vector<uint64_t> *, 4> {
