...
auto fused_find_and_collect(const MPOperator<NumModes> &op,
...

    {
        ...
        auto push = [&](const Monomial<NumModes> &dense,
            ...
            // routing::Router is the ONLY owner function -- find_rank (MPIUtils.h) must stay in step
            // with it, or a term is placed and queried on different ranks, which duplicates a row
            // silently; mpi_utils_tests.cpp asserts the agreement.
