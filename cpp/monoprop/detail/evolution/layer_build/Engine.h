...
struct LayerBuildEngine {
    ...
    // The gate's transient buffers at one instant. Every one of them dies when the gate returns, so no
    // resting field of the ledger can name them -- and on a wide gate they are the largest thing the call
    // adds to RSS. That is why this is kept as a high-water mark over the call and not as a resting field.
    //
    // scan.cos_blocks is excluded deliberately: it is moved into the layer rather than freed, so the layer
    // storage already prices it. `extra` carries round 2's answers, which are live only at the second
    // sample.
