...
// Queries ride flat VecZ buffers. There is one record format -- CompactQuery, the ascending set-bit
// positions -- and it is VARIABLE WIDTH: a term wider than six positions continues into further
// records, so the q'th query does NOT start at q * <constant>. Nothing here may reintroduce a stride.
// Every offset comes from QueryCodec, which derives it by walking. The source index is not in the
// payload — the resolver answers by position; the querier holds src_idx_r[r][q].
