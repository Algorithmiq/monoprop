...
public:
    ...
    auto find_batch_positions(const PosT *pos_flat,
        ...
        for (size_t base = 0; base < n; base += G) {
            ...
            for (size_t j = 0; j < g; ++j) {
                ...
                const size_t i = slot_idx_(cand[j]);
                if (row_eq_positions(i, qpos, qk)) {
                    out[q] = i;
                }
                else {
                    // A fingerprint collision: rare enough to walk the chain from the top rather than
                    // resume it.
                    out[q] = find_positions_(sp[j], qpos, qk);
                }
            }
        }
    }
    ...
    auto bulk_insert_hashed(size_t n, mapped_type base, HashFn &&hash_at) -> void {
        ...
        for (size_t b = 0; b < n; b += G) {
            ...
            for (size_t j = 0; j < g; ++j) {
                sp[j] = spread(hash_at(b + j));
                __builtin_prefetch(slot_addr_(sp[j] & table_.mask), /*rw=*/1, /*locality=*/0);
            }
            for (size_t j = 0; j < g; ++j) {
                insert_slot_(static_cast<TermIndex>(base + b + j), sp[j]);
            }
        }
    }
...
private:
    ...
    [[nodiscard]] auto find_positions_(size_t sp, const PosT *q, size_t qk) const -> size_t {
        if (table_.count == 0) {
            return kNotFound;
        }
        const uint8_t f = fingerprint(sp);
        for (size_t s = sp & table_.mask;; s = (s + 1) & table_.mask) {
            ...
            if (e == f && row_eq_positions(slot_idx_(s), q, qk)) {
                return slot_idx_(s);
            }
        }
    }
