...
struct GraphMemoryBreakdown final {
    ...
    // Diagnostics, deliberately OUTSIDE total_bytes(): each is either a count or a slice of
    // cross_rank_bytes, so adding them would double-count. They exist to separate the part of the
    // graph that scales with the flat world size P from the part that scales with traffic.
