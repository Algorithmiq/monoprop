...
BOOST_AUTO_TEST_CASE(validation_weight_refresh) {
    const WeightRefresh followable{.weights_revision = 3, .expected_revision = 3, .may_follow_weights = true};
    BOOST_CHECK_NO_THROW(validate_weight_refresh(followable));

    // What a pared Schrodinger functional reports: its keep-set came from the coefficients the re-weight
    // replaced, so replaying it would answer for a paring nobody asked for.
    auto refuses = followable;
    refuses.may_follow_weights = false;
    BOOST_CHECK_EXCEPTION(validate_weight_refresh(refuses), std::runtime_error, [](const auto &e) {
