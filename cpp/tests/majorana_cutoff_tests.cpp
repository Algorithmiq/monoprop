...
TEST_CASE("majorana_cutoff_paired_kept_unconditionally") {
    ...
    paired.set(4);
    paired.set(5);
    CHECK(is_paired<N>(paired));
    CHECK(length_cutoff<N>(paired, 0));
    CHECK(length_cutoff<N>(paired, 2));
    CHECK(support_cutoff<N>(paired, 0));
}
...
TEST_CASE("majorana_cutoff_length_and_support_thresholds") {
    ...
    unpaired.set(2);
    unpaired.set(4);
    CHECK(!is_paired<N>(unpaired));

    // length = popcount = 3; support (distinct orbitals) = 3 here.
    CHECK(length_cutoff<N>(unpaired, 3));
    CHECK(!length_cutoff<N>(unpaired, 2));
    CHECK(support_cutoff<N>(unpaired, 3));
    CHECK(!support_cutoff<N>(unpaired, 2));
    ...
    for (int trial = 0; trial < 400; ++trial) {
        ...
        for (unsigned int c : {0U, 1U, 2U, 3U}) {
            if (length_cutoff<N>(m, c)) {
                CHECK(support_cutoff<N>(m, c));
            }
        }
        CHECK(length_cutoff<N>(m, 2 * N));
        CHECK(length_cutoff<N>(m, 0) == is_paired<N>(m));
    }
}

// logical_num_modes masks off the inactive low-mode prefix, so bits there must not count against the
// active window.
TEST_CASE("majorana_cutoff_logical_num_modes_masks_prefix_single_word") {
    constexpr size_t N = 32;
    ...
    CHECK(length_cutoff<N>(prefix_only, 0, logical));
    CHECK(support_cutoff<N>(prefix_only, 0, logical));
    // Over the whole register the lone bit is unpaired and exceeds cutoff 0 -> dropped.
    CHECK(!length_cutoff<N>(prefix_only, 0, N));
    CHECK(!length_cutoff<N>(prefix_only, 0)); // whole-register overload

    Monomial<N> active_bit;
    active_bit.set(52);
    CHECK(!length_cutoff<N>(active_bit, 0, logical));
    Monomial<N> active_pair;
    active_pair.set(52);
    active_pair.set(53);
    CHECK(length_cutoff<N>(active_pair, 0, logical));
}

TEST_CASE("majorana_cutoff_logical_num_modes_masks_prefix_multi_word") {
    ...
    CHECK(length_cutoff<N>(prefix_only, 0, logical)); // active window empty -> kept
    CHECK(!length_cutoff<N>(prefix_only, 0, N));      // whole register -> dropped
}

TEST_CASE("majorana_cutoff_evaluator_dispatch_and_popcount") {
    constexpr size_t N = 32;

    CutoffFn<N> length_fn = detail::LengthCutoff<N>{.cutoff = 3};
    detail::CutoffEvaluator<N> length_ev(length_fn);
    CHECK((length_ev.length_cutoff() != nullptr));
    CHECK((length_ev.support_cutoff() == nullptr));
    REQUIRE(length_ev.max_slot_bound().has_value());
    // A length cutoff counts set bits directly, so the slot bound IS the cutoff.
    CHECK(length_ev.max_slot_bound().value() == 3U);

    CutoffFn<N> support_fn = detail::SupportCutoff<N>{.cutoff = 2};
    detail::CutoffEvaluator<N> support_ev(support_fn);
    CHECK((support_ev.length_cutoff() == nullptr));
    CHECK((support_ev.support_cutoff() != nullptr));
    // A support cutoff counts modes/qubits and each spans two slots, so the slot bound doubles.
    CHECK(support_ev.max_slot_bound().value() == 4U);

    CutoffFn<N> opaque_fn = [](const Monomial<N> &) { return true; };
    detail::CutoffEvaluator<N> opaque_ev(opaque_fn);
    CHECK((opaque_ev.length_cutoff() == nullptr));
    CHECK((opaque_ev.support_cutoff() == nullptr));
    CHECK(!opaque_ev.max_slot_bound().has_value());
    ...
    unpaired.set(4);
    unpaired.set(6);
    CHECK(length_ev.passes_with_popcount(unpaired, 3));
    CHECK(!length_ev.passes_with_popcount(unpaired, 4));
    CHECK(length_ev.passes_with_popcount(unpaired, 4) == length_ev(unpaired));
    ...
    paired.set(2);
    paired.set(3);
    CHECK(length_ev.passes_with_popcount(paired, 10));
}

// interleave_phase (reference prefix-XOR scan) must equal the masked-parity form used on the hot path.
TEST_CASE("majorana_cutoff_interleave_phase_mask_cross_check") {
    auto check = [](auto tag) {
...
TEST_CASE("majorana_cutoff_encode_decode_coeff") {
    constexpr size_t N = 32;
    Monomial<N> mono;
    mono.set(0);
    mono.set(3);
    ...
    for (double r : {1.0, -2.5, 0.0, 7.25}) {
        ...
        CHECK(encode_coeff<N>(hermitian, mono) == r);
    }
    ...
    CHECK_THROWS_AS(encode_coeff<N>(non_hermitian, mono), std::runtime_error);
}
...
TEST_CASE("majorana_cutoff_paired_op_saturates_at_one_pair_per_mode") {
    constexpr size_t N = 32;
