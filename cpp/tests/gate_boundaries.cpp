...
TEST_CASE("n_gates_defaults_to_one_per_generator") {
    auto sim = make_sim();
    const std::vector<VecZ> monos{{0}, {1}, {2}};
    sim.build_graph(monos, VecZ{0, 1, 2}, VecD{1.0, 1.0, 1.0});
    CHECK(sim.graph_layers() == 3u);
    CHECK(sim.n_gates() == sim.graph_layers());
}

TEST_CASE("gate_indices_group_layers") {
    auto sim = make_sim();
    const std::vector<VecZ> monos{{0}, {1}, {2}};
    // Two monomials belong to gate 0, one to gate 1.
    sim.build_graph(monos, VecZ{0, 1, 2}, VecD{1.0, 1.0, 1.0}, VecZ{0, 0, 1});
    CHECK(sim.graph_layers() == 3u);
    CHECK(sim.n_gates() == 2u);
}

TEST_CASE("set_parameter_mapping_per_gate_ties_layers") {
    auto sim = make_sim();
    const std::vector<VecZ> monos{{0}, {1}, {2}};
    sim.build_graph(monos, VecZ{0, 1, 2}, VecD{1.0, 1.0, 1.0}, VecZ{0, 0, 1});
    ...
    CHECK(per_layer.size() == 3u);
    CHECK(std::ranges::all_of(per_layer, [](size_t p) { return p == 0; }));
...
TEST_CASE("set_parameter_mapping_per_layer_still_works") {
    auto sim = make_sim();
    const std::vector<VecZ> monos{{0}, {1}, {2}};
    sim.build_graph(monos, VecZ{0, 1, 2}, VecD{1.0, 1.0, 1.0}, VecZ{0, 0, 1});
    ...
    CHECK(sim.parameter_mapping().size() == 3u);
    CHECK(std::ranges::all_of(sim.parameter_mapping(), [](size_t p) { return p == 0; }));
}
...
TEST_CASE("set_parameter_mapping_after_gradient_matches_before") {
    const std::vector<VecZ> monos{{0}, {1}, {2}};
    ...
    CHECK((value_after) == Catch::Approx(value_before).epsilon(1e-12));
    REQUIRE((grad_after.size()) == (grad_before.size()));
    for (size_t i = 0; i < grad_before.size(); ++i) {
        CHECK((grad_after[i]) == Catch::Approx(grad_before[i]).epsilon(1e-12));
    }
}

TEST_CASE("set_parameter_mapping_rejects_bad_length") {
    auto sim = make_sim();
    const std::vector<VecZ> monos{{0}, {1}, {2}};
    sim.build_graph(monos, VecZ{0, 1, 2}, VecD{1.0, 1.0, 1.0}, VecZ{0, 0, 1});
    // Length 4 matches neither graph_layers (3) nor n_gates (2).
    CHECK_THROWS_AS(sim.set_parameter_mapping(VecZ{0, 1, 2, 3}), std::runtime_error);
}

TEST_CASE("n_gates_accumulates_across_builds") {
    auto sim = make_sim();
    sim.build_graph(std::vector<VecZ>{{0}}, VecZ{0}, VecD{1.0});
    CHECK(sim.n_gates() == 1u);
...
TEST_CASE("build_graph_rejects_malformed_gate_indices") {
    auto sim = make_sim();
    const std::vector<VecZ> monos{{0}, {1}};
    // A jump from 0 to 2 is not a contiguous run.
    CHECK_THROWS_AS(sim.build_graph(monos, VecZ{0, 1}, VecD{1.0, 1.0}, VecZ{0, 2}), std::runtime_error);
}

TEST_CASE("coeff_informed_build_graph_rejects_too_few_parameters") {
    auto sim = make_sim();
...
TEST_CASE("contract_partially_replays_existing_graph_and_supports_inplace") {
    auto sim = make_sim();
    sim.build_graph(std::vector<VecZ>{{0}, {1}}, VecZ{0, 1}, VecD{1.0, 1.0});
    CHECK(sim.graph_layers() == 2u);
    ...
    CHECK(consumed.size() == peeked.size());
    CHECK(sim.graph_layers() == 0u);
}
