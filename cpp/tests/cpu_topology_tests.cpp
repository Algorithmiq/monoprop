...
auto empty_placement_is_licensed() -> bool {
    ...
    INFO("monoprop_PARTITION_PINNING is off; partition_cpusets places nothing");
    return true;
}
...
TEST_CASE("cpu_topology_enumerate_and_place") {
    ...
    CHECK(too_many.empty());
}

TEST_CASE("cpu_topology_place_co_located_ranks") {
    const auto cores = partition::enumerate_physical_cores();
    if (cores.size() < 2) {
    ...
    REQUIRE((rank0.size()) == (1u));
    REQUIRE((rank1.size()) == (1u));
    ...
    CHECK(rank0.front().pu != rank1.front().pu);
...
TEST_CASE("cpu_topology_policy_disjoint_mpi_ranks") {
    ...
    REQUIRE((r0.size()) == (1u));
    REQUIRE((r1.size()) == (1u));
    CHECK(r0.front() != r1.front());
}

TEST_CASE("cpu_topology_policy_domain_major_more_ranks_than_l3") {
...
TEST_CASE("cpu_topology_policy_insufficient_cores_returns_empty") {
    ...
    CHECK(placement_order(cores, 2, 0, 2).empty());
    CHECK(placement_order(cores, 2, 1, 2).empty());

    // Single rank requesting more cores than exist.
    CHECK(placement_order(cores, 3, 0, 1).empty());

    // Empty core list.
    CHECK(placement_order({}, 1, 0, 1).empty());
}

TEST_CASE("cpu_topology_policy_singleton_l3_domains") {
...
TEST_CASE("cpu_topology_policy_uneven_domains") {
...
TEST_CASE("cpu_topology_policy_per_rank_slice_starves_without_collapse") {
    ...
    CHECK(placement_order(slice, 2, /*group_index=*/3, /*group_count=*/8).empty());
...
TEST_CASE("cpu_topology_policy_private_mask_collapses_even_when_the_split_would_fit") {
...
TEST_CASE("cpu_topology_masks_disjoint_vs_identical") {
    constexpr size_t kWords = partition::kAffinityMaskWords;

    const auto disjoint = packed_masks({{0, 1}, {2, 3}}, kWords);
    CHECK(partition::masks_are_pairwise_disjoint(disjoint.data(), 2, kWords));

    const auto identical = packed_masks({{0, 1}, {0, 1}}, kWords);
    CHECK(!partition::masks_are_pairwise_disjoint(identical.data(), 2, kWords));
    ...
    CHECK(!partition::masks_are_pairwise_disjoint(partial.data(), 2, kWords));
    ...
    CHECK(!partition::masks_are_pairwise_disjoint(with_empty.data(), 2, kWords));

    const auto lone = packed_masks({{0, 1}}, kWords);
    CHECK(!partition::masks_are_pairwise_disjoint(lone.data(), 1, kWords));

    const auto four_ok = packed_masks({{0}, {1}, {2}, {3}}, kWords);
    CHECK(partition::masks_are_pairwise_disjoint(four_ok.data(), 4, kWords));
    const auto four_bad = packed_masks({{0}, {1}, {2}, {1}}, kWords);
    CHECK(!partition::masks_are_pairwise_disjoint(four_bad.data(), 4, kWords));
}

TEST_CASE("cpu_topology_masks_span_word_boundaries") {
    constexpr size_t kWords = partition::kAffinityMaskWords;

    // A per-word comparison that forgot to loop would answer from word 0 alone.
    const auto low_high = packed_masks({{5}, {200}}, kWords);
    CHECK(partition::masks_are_pairwise_disjoint(low_high.data(), 2, kWords));

    const auto both_high = packed_masks({{200}, {200}}, kWords);
    CHECK(!partition::masks_are_pairwise_disjoint(both_high.data(), 2, kWords));

    const auto late_overlap = packed_masks({{1, 3000}, {2, 3000}}, kWords);
    CHECK(!partition::masks_are_pairwise_disjoint(late_overlap.data(), 2, kWords));
}

TEST_CASE("cpu_topology_affinity_mask_covers_enumerated_cores") {
    ...
    if (!partition::affinity_mask_words(mine.data(), mine.size())) {
        ...
        REQUIRE(partition::affinity_mask_words(wide.data(), wide.size()));
        size_t highest = 0;
        ...
        CHECK((highest) >= (partition::kAffinityMaskWords * 64));
        return;
    }
    size_t set_bits = 0;
...
auto confine_to_first(const std::vector<partition::PhysicalCore> &full, size_t k) -> bool {
    ...
    REQUIRE((partition::enumerate_physical_cores().size()) == (k));
    return true;
}

} // namespace

TEST_CASE("cpu_topology_per_rank_mask_still_places") {
    ...
    if (k == 2) {
        CHECK(sets[0].pu != sets[1].pu);
    }
}

TEST_CASE("cpu_topology_shared_mask_keeps_co_located_ranks_disjoint") {
    const auto full = partition::enumerate_physical_cores();
    if (full.size() < 2) {
    ...
    REQUIRE((rank0.size()) == (per_rank));
    REQUIRE((rank1.size()) == (per_rank));
