...
TEST_CASE("indices_to_bitset_test") {
    const auto index = GENERATE(0U, 1U, 2U, 3U);
    const auto& input_indices = ds_input_indices_to_bitset_test[index];
    const auto& expected_bitset = ds_output_indices_to_bitset_test[index];
    CAPTURE(index);
    auto bitset = indices_to_bitset<NumQubits>(input_indices);
    CHECK(bitset == expected_bitset);
}
...
TEST_CASE("length_cutoff_test") {
    const auto index = GENERATE(0U, 1U, 2U);
    const auto& input_bitset = ds_input_bitset_to_indices_test[index];
    const auto cutoff = ds_cutoff_values[index];
    const auto expected_result = ds_expected_length_results[index];
    CAPTURE(index);
    auto result = length_cutoff<NumQubits>(input_bitset, cutoff);
    CHECK(result == expected_result);
}

TEST_CASE("test_fermionic_to_binary_operator_empty") {
...
TEST_CASE("test_fermionic_to_binary_operator_single_term") {
    ...
    CHECK(result.size() == 1);
    CHECK(result[0] == 0b11100000);
}

TEST_CASE("test_fermionic_to_binary_operator_multiple_terms") {
    std::vector<VecZ> multi_term_operator = {{0, 1}, {2, 3}};
    auto result = fermionic_to_binary_operator<NumQubits>(multi_term_operator);
    CHECK(result.size() == 2);
    CHECK(result[0] == 0b11000000);
    CHECK(result[1] == 0b00110000);
}
...
TEST_CASE("get_multiplicative_phase_test") {
    const auto& test_pair = GENERATE_REF(Catch::Generators::from_range(ds_get_multiplicative_phase));
...
TEST_CASE("is_fully_paired_test") {
    const auto& test_case = GENERATE_REF(Catch::Generators::from_range(ds_is_fully_paired_test));
    CAPTURE(test_case.test_name);
    auto result = is_fully_paired<NumQubits2>(test_case.inds, test_case.op_terms);
    CHECK(std::is_permutation(result.cbegin(), result.cend(), test_case.expected_result.cbegin()));
}

TEST_CASE("bit_flipping_utilities") {
    auto val1 = even_bits<10, LSb0>();
    auto val2 = odd_bits<10, LSb0>();
    auto val3 = even_bits<10, MSb0>();
    auto val4 = odd_bits<10, MSb0>();
    CHECK(val1 == 0b0101010101);
    CHECK(val2 == 0b1010101010);
    CHECK(val3 == 0b1010101010);
    CHECK(val4 == 0b0101010101);
}
...
TEST_CASE("eval_state_sparse_dot_is_bit_identical_to_dense") {
    ...
    CHECK((sparse.length()) == (kStateLength));
    CHECK((sparse.dot(op)) == (inner_product(dense, op)));
    CHECK((EvalState::dense(dense).dot(op)) == (inner_product(dense, op)));
    ...
    CHECK((empty.length()) == (kStateLength));
    CHECK((empty.dot(op)) == (0.0));

    // An operator longer than the state is fine (dot spans the state); shorter is a hard error.
    VecD longer = op;
    longer.push_back(1.0);
    CHECK((sparse.dot(longer)) == (sparse.dot(op)));
    const VecD shorter(kStateLength - 1, 1.0);
    CHECK_THROWS_AS(sparse.dot(shorter), std::invalid_argument);
}
...
TEST_CASE("eval_state_indices_above_matches_the_dense_scan") {
    const auto dense = make_dense(kStateLength, kRows, kVals);
    const auto sparse = EvalState::sparse(kStateLength, kRows, kVals);
    ...
    for (const auto t : thresholds) {
        {
            ...
            CHECK(sparse.indices_above(t) == expected);
            CHECK(EvalState::dense(dense).indices_above(t) == expected);
        }
    }

    // Spot-check the two ends rather than trusting the dense oracle alone.
    CHECK((sparse.indices_above(0.0).size()) == (kRows.size()));
    CHECK((sparse.indices_above(-1.0).size()) == (kStateLength));
    CHECK(sparse.indices_above(1.0).empty());
    CHECK(sparse.indices_above(std::numeric_limits<double>::quiet_NaN()).empty());
}
...
TEST_CASE("sparse_energy_matches_the_dense_gradient_value_bit_exactly") {
    ...
    for (const auto schrodinger_cutoff : {std::optional<unsigned int>{}, std::optional<unsigned int>{4}}) {
        {
            INFO("schrodinger_cutoff = " << (schrodinger_cutoff ? "4" : "none"));
            ...
            CHECK((sim.expectation_value(data.parameters))
            ...
            CHECK((sim.expectation_value_functional(threshold)(data.parameters))
...
TEST_CASE("interleaved_gradients_do_not_share_scratch_state") {
    ...
    auto narrow = build(4);
    REQUIRE(wide.mp_op().size() != narrow.mp_op().size());
    ...
    for (int round = 0; round < 2; ++round) {
        {
            ...
            CHECK((wide_value) == (ref_wide_value));
            CHECK(wide_grad == ref_wide_grad);

            const auto [narrow_value, narrow_grad] = grad_narrow(data.parameters);
            CHECK((narrow_value) == (ref_narrow_value));
            CHECK(narrow_grad == ref_narrow_grad);
        }
    }

    // The sparse energy path leaves no dense state behind on the Heisenberg operator.
    CHECK((wide.expectation_value_functional()(data.parameters)) == (ref_wide_value));
    CHECK(wide.mp_op().state_coeffs.empty());
    CHECK(narrow.mp_op().state_coeffs.empty());
}

TEST_CASE("eval_state_sparse_rejects_inconsistent_inputs") {
    CHECK_THROWS_AS(EvalState::sparse(kStateLength, kRows, VecD{1.0}), std::invalid_argument);
    const std::vector<TermIndex> out_of_range = {0, static_cast<TermIndex>(kStateLength)};
    CHECK_THROWS_AS(EvalState::sparse(kStateLength, out_of_range, VecD{1.0, 1.0}), std::invalid_argument);
    CHECK_NOTHROW(EvalState::sparse(0, {}, {}));
}
