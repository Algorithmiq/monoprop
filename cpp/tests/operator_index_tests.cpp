...
TEST_CASE("rows_roundtrip_dense_popcount_positions") {
    Store s;
    s.push_back(bs({0, 3, 5}));
    s.push_back(bs({1, 2}));
    CHECK(s.size() == 2u);
    CHECK(s.popcount(0) == 3u);
    CHECK(s.popcount(1) == 2u);
    CHECK((s.row(0) == bs({0, 3, 5})));
...
TEST_CASE("index_emplace_then_find_roundtrip") {
    Store s;
    s.push_back(bs({0, 3, 5}));
    ...
    CHECK(f.has_value());
    CHECK(*f == 1u);
    CHECK(!s.find(bs({7, 9})).has_value());
}

TEST_CASE("width_is_a_construction_invariant") {
    ...
    CHECK(s.popcount(0) == 4u);
    CHECK((s.row(0) == bs({0, 2, 4, 6})));
}

TEST_CASE("overflow_is_lossless_above_width") {
    Store s(2); // width 2; a 3-position row must overflow
    s.push_back(bs({0, 1, 2}));
    CHECK(s.popcount(0) == 3u); // popcount recovered from the overflow map
    CHECK((s.row(0) == bs({0, 1, 2})));
}

TEST_CASE("index_survives_rehash_in_place") {
    Store a;
...
TEST_CASE("clone_is_deep_and_independent") {
...
TEST_CASE("clone_preserves_overflow_rows") {
    Store a(2); // width 2; a 3-position row overflows losslessly
    a.push_back(bs({0, 1, 2}));
    a.emplace(bs({0, 1, 2}), 0);

    auto b = a.clone();
    CHECK(b->popcount(0) == 3u);
    CHECK((b->row(0) == bs({0, 1, 2})));
    CHECK(*b->find(bs({0, 1, 2})) == 0u);
}
...
TEST_CASE("find_batch_matches_scalar_find") {
    Store s;
    ...
    CHECK(all_match);
    CHECK(out[0] == 0u);               // first present key -> row 0
    CHECK(out[1] == Store::kNotFound); // first absent key
}

// Pins find_batch's partition.count == 0 early-out.
TEST_CASE("find_batch_on_empty_store_is_all_missing") {
    Store s;
