...
TEST_CASE("pruned_layer_supports_cos_counts_above_u32") {
    ...
    CHECK((lt.num_cos_inds()) == (large_count));
...
TEST_CASE("cross_rank_partner_range_counts_track_term_index_width") {
    CrossRankPartnerRange r{};
    CHECK((sizeof(r.sin_send_count)) == (sizeof(TermIndex)));
    CHECK((sizeof(r.sin_recv_count)) == (sizeof(TermIndex)));
    CHECK((sizeof(r.in_count)) == (sizeof(TermIndex)));
}
...
TEST_CASE("cross_rank_sin_send_index_round_trips_above_u32") {
    ...
    CHECK((detail::cross_rank_sin_send_index(storage, 1, 0)) == (big_in));
    CHECK((detail::cross_rank_sin_send_index(storage, 1, 1)) == (big_out));
    // D[0] is derived from B: Q = sin_recv_count - in_count = 1, so D[0] = out-block[0] = big_out.
    CHECK((detail::cross_rank_sin_recv_index(storage, 1, 0)) == (big_out));
}
#endif
...
TEST_CASE("checked_mpi_int_throws_cleanly_above_int_max") {
    const size_t at_limit = static_cast<size_t>(std::numeric_limits<int>::max());
    CHECK((detail::checked_mpi_int(at_limit, "exchange count")) == (std::numeric_limits<int>::max()));
    CHECK_THROWS_AS(detail::checked_mpi_int(at_limit + 1, "exchange count"), std::overflow_error);
}

TEST_CASE("cosine_word_list_scale_and_accumulate") {
    ...
    CHECK((a_par) == Catch::Approx(5.0 * 1.5 * 0.5).epsilon(1e-12));
    // state and ham at set indices scaled by cos_val and sec_val respectively
    CHECK((pp[0]) == Catch::Approx(1.5 * 0.7).epsilon(1e-12));
    CHECK((ph[0]) == Catch::Approx(0.5 * 0.9).epsilon(1e-12));
}
...
TEST_CASE("packed_cross_rank_storage_bit_packs_binary_phases") {
    ...
    CHECK((detail::cross_rank_sin_recv_phase(binary_storage, 1, 1)) == (1));
    // D^+[0] = term idx+1005=1005, phase +1, stored as-is. (D^+ starts at flat index 128.)
    CHECK((detail::cross_rank_sin_recv_phase(binary_storage, 1, 128)) == (1));

    CHECK((detail::cross_rank_storage_bytes(binary_storage)) < (detail::cross_rank_storage_bytes(wide_phase_storage)));
}
