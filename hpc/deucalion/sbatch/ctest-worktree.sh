...
if [ "${SYNC:-1}" = 1 ]; then
    uv sync --all-extras --group test --group bench \
        ...
        --config-settings-package="monoprop:cmake.define.monoprop_MAX_NUM_MODES=${MAX_NUM_MODES:-1024}" \
        --config-settings-package="monoprop:build-dir=build/{state}/{build_type}-$TAG" \
        --config-settings-package="monoprop:build.tool-args=-j${JOBS:-32}" \
...
cmake -S . -B "$BUILD" -DCMAKE_BUILD_TYPE=Release \
    ...
    -Dmonoprop_ENABLE_MPI=ON -Dmonoprop_MAX_NUM_MODES="${MAX_NUM_MODES:-1024}" \
...
cmake --build "$BUILD" -j"${JOBS:-32}" || { echo "!! cmake build FAILED"; exit 1; }
