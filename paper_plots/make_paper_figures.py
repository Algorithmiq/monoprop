...
def _two_part_legend(
    fig, cutoffs, *, engines=("monoprop", "julia"), y_cut=0.075, y_eng=0.015
):
...
def _save(fig, outdir: Path, stem: str):
    ...
    for ext, dpi in (("pdf", None), ("png", PNG_DPI)):
        ...
        # CreationDate=None drops the wall-clock stamp the PDF backend writes by default, so a
        # rebuild from the same data is byte-identical -- as in node_scaling/.
...
def fig2_divergence_scaling(records, outdir):
    """Julia ÷ monoprop: the divergence, and how it scales."""
    cutoffs = sorted({r["cutoff"] for r in records})
    fig, axes = plt.subplots(1, 2, figsize=(9.6, 4.3))
    panels = [
        ("seconds", "time overhead  ($\\times$ monoprop)"),
...
def fig4_scaling_and_divergence(records, layers, outdir):
    ...
    rat_panels = [
        ("seconds", "time overhead  ($\\times$ monoprop)", [1, 2]),
...
def main() -> None:
    ...
    outputs += fig2_divergence_scaling(records, args.outdir)
    ...
    for metric, name in (("seconds", "time"), ("memory_mb", "memory")):
        for c in cutoffs:
            ...
            mono = dict(zip(mx, my, strict=True))
            jul = dict(zip(jx, jy, strict=True))
            xs = sorted(set(mono) & set(jul))
            pr = _fit_exponent(xs, [jul[x] / mono[x] for x in xs])
            if pm and pj:
                print(
                    f"  {name} c{c}: mono N^{pm:.2f}  julia N^{pj:.2f}  "
