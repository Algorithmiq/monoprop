...
def load(path=DATA):
    ...
    with pathlib.Path(path).open() as fh:
        for r in csv.DictReader(fh, delimiter="\t"):
            ...
            e = rungs.setdefault(
                key, {"secs": [], "terms": int(r["terms"]), "nodes": int(r["nodes"])}
            )
...
def nodes_axis(ax, cores, label=True):
    ...
    top = ax.secondary_xaxis(
        "top", functions=(lambda c: c / CORES_PER_NODE, lambda n: n * CORES_PER_NODE)
    )
    top.xaxis.set_major_locator(FixedLocator([c / CORES_PER_NODE for c in cores]))
    top.xaxis.set_major_formatter(FuncFormatter(lambda v, _: f"{round(v)}"))
...
def draw_strong_time(ax, curves, legend=True):
    ...
    if legend and curve_legend(ax, "lower left"):
        clip_y(
            ax, curves, lambda p: p["median"]
        )  # stacked instead: give the strip back
...
def draw_strong_efficiency(ax, curves, legend=True):
    ...
    for i, (lab, pts) in enumerate(curves):
        ...
        line(
            ax,
            [p["cores"] for p in pts],
            [100.0 * t0 * n0 / (p["median"] * p["nodes"]) for p in pts],
            i,
            lab,
        )
...
def draw_weak_efficiency(ax, curves, legend=True):
    ...
    for i, (lab, pts) in enumerate(curves):
        ...
        line(
            ax,
            [p["cores"] for p in pts],
            [100.0 * t0 / p["median"] for p in pts],
            i,
            lab,
        )
    ax.set_ylim(0, 108)
...
def fig_combined(rungs, outdir):
    ...
    cols = [
        ("Strong scaling", strong, draw_strong_time, draw_strong_efficiency),
...
def main():
    ...
    missing = [lab for lab in STRONG + WEAK if lab not in have]
    if missing:
        print(f"  NOT MEASURED (curve omitted, not drawn short): {', '.join(missing)}")
    drawn = {
        stem: fig_single(rungs, outdir, kind, stem, draw, logy)
