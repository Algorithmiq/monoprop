...
def strong_time(curves):
    ...
    return (
        f"Strong scaling of the Hubbard propagate operator: wall time against core count, "
...
def strong_efficiency(curves):
    ...
    return (
        f"Strong-scaling parallel efficiency, t(N0)N0/t(N)N, for the same runs. Each curve is "
...
def weak_time(curves):
    ...
    for lab, pts in curves:
        ...
        loads.append(
            f"{sum(pn) / len(pn):.0f}M ({min(pn):.0f}–{max(pn):.0f}M measured)"
        )
    return (
        f"Weak scaling of the same operator: wall time against core count, {lo} to {hi} "
...
def weak_efficiency(curves):
    ...
    return (
        f"Weak-scaling parallel efficiency, t({lo} cores) / t(N), for the same runs. At {hi} "
...
def combined(drawn):
    ...
    return (
        f"Strong and weak scaling of the Hubbard propagate operator on {lo} to {hi} cores "
