...
// mutating one instance must never affect the other. The operator store owns a self back-pointer
// (OperatorIndex::RowEq), so the copy rebuilds that store via clone() with the pointer repaired --
// find()/indexing() therefore have to work on the copy. The MPI communicator handle is shared (not
// dup'd). This is the mechanism behind Python __deepcopy__.
...
BOOST_FIXTURE_TEST_CASE(copy_constructed_simulator_matches_energy, ExampleDataFix) {
    ...
    const double e_orig = sim.expectation_value_functional(data.param_inds, data.gen_coeffs)(data.parameters);
    const double e_copy = copy.expectation_value_functional(data.param_inds, data.gen_coeffs)(data.parameters);
...
BOOST_FIXTURE_TEST_CASE(copy_is_independent_of_source, ExampleDataFix) {
    ...
    copy.propagate(data.majoranas);
    const double e_sim = sim.expectation_value_functional(data.param_inds, data.gen_coeffs)(data.parameters);
    const double e_copy = copy.expectation_value_functional(data.param_inds, data.gen_coeffs)(data.parameters);
...
BOOST_FIXTURE_TEST_CASE(copy_graph_survives_other_being_contracted_and_destroyed, ExampleDataFix) {
    ...
    original.propagate(data.majoranas);

    const size_t layers_before = original.graph_layers();
    BOOST_TEST(layers_before > 0u);
    const double e_before =
        original.expectation_value_functional(data.param_inds, data.gen_coeffs)(data.parameters);

    {
        ...
        copy.contract_partially(data.parameters, data.param_inds, data.gen_coeffs, /*inplace=*/true);
...
BOOST_FIXTURE_TEST_CASE(copy_constructed_simulator_index_backpointer_valid, ExampleDataFix) {
    SimulatorConfig cfg{.comm = MPI_COMM_SELF};
    auto sim = build_simulator<n_modes>(data, cfg);
    sim.propagate(data.majoranas);
    ...
    // Every stored term must round-trip through the copy's own index (proves the back-pointer was
    // repaired: find confirms hash hits against the copy's rows, not the source's).
