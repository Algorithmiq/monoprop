...
// CutoffContext::is_above_upper and the emit gate in DistributedLayerBuilder.h.
...
auto build_zero_cutoff_full_rescue(const MPData& data, MPI_Comm comm) -> MonomialPropagator<NumModes> {
...
template <size_t NumModes>
auto evaluate_zero_cutoff_full_rescue_energy(MonomialPropagator<NumModes>& simulator, const MPData& data)
    -> double {
    simulator.propagate(data.majoranas, data.param_inds, data.gen_coeffs, data.parameters);
    auto energy_fn = simulator.expectation_value_functional(VecZ{}, VecD{}, std::nullopt);
    return energy_fn(VecD{});
}

struct RandomExactFixture {
    static constexpr size_t n_modes = 8;
    monoprop::MPData data;
