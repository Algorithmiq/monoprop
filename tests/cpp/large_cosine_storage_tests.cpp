...
BOOST_AUTO_TEST_CASE(pruned_layer_supports_cos_counts_above_u32) {
    ...
    for (size_t idx = 0; idx < 12; ++idx) {
        p.b_indices.push_back(200 + idx);
    }
    for (size_t idx = 0; idx < 8; ++idx) {
        p.b_indices.push_back(100 + idx);
    }
    // Single phased D list: D- (out) block first with negated phase, then D+ (in) block.
    for (size_t idx = 0; idx < 8; ++idx) {
        p.d.push_back({100 + idx, -(idx % 2 == 0 ? 1 : -1)});
    }
    for (size_t idx = 0; idx < 12; ++idx) {
        p.d.push_back({200 + idx, idx % 2 == 0 ? -1 : 1});
    }
    ...
    Layer layer{LayerKind{PrunedLayer{storage, std::move(pruned_cos)}}};

    // The >u32 cosine count round-trips through the stored pruned cos (CosineWordList::total_count).
    ...
    layer.for_each_cross_rank_b_range(1, 0, 1, [&](size_t, size_t i) { b_idx = i; });
    BOOST_CHECK_EQUAL(b_idx, 200UL);

    // D[0] is derived from B: Q = d_count - in_count = 20 - 12 = 8, so D[0] = out-block[0] = 100,
    // stored phase = -(out_phases[0]) = -(+1) = -1.
    size_t d_idx = static_cast<size_t>(-1);
    int    d_phi = 0;
    layer.for_each_cross_rank_d_range(1, 0, 1, [&](size_t, size_t i, int phi) { d_idx = i; d_phi = phi; });
...
BOOST_AUTO_TEST_CASE(cross_rank_partner_range_counts_track_term_index_width) {
    ...
    BOOST_CHECK_EQUAL(sizeof(r.b_count), sizeof(TermIndex));
    BOOST_CHECK_EQUAL(sizeof(r.d_count), sizeof(TermIndex));
...
BOOST_AUTO_TEST_CASE(cross_rank_b_index_round_trips_above_u32) {
    ...
    p.b_indices.push_back(big_in);
    p.b_indices.push_back(big_out);
    p.d.push_back({big_out, -1}); // D- (out) block, stored -phase
    p.d.push_back({big_in, 1});   // D+ (in) block
    p.in_count = 1;
    ...
    BOOST_CHECK_EQUAL(detail::cross_rank_b_index(storage, 1, 0), big_in);
    BOOST_CHECK_EQUAL(detail::cross_rank_b_index(storage, 1, 1), big_out);
    // D index 0 is derived from B: D[0] = out-block[0] = big_out (Q = d_count - in_count = 1).
    BOOST_CHECK_EQUAL(detail::cross_rank_d_index(storage, 1, 0), big_out);
}
#endif
...
BOOST_AUTO_TEST_CASE(cosine_word_list_scale_and_accumulate) {
    ...
    monoprop::detail::scale_cos_words(par.data(), cos, 3.0);
    // Exactly the set indices were multiplied by 3.0, nothing else.
    BOOST_TEST(par[0] == 6.0); BOOST_TEST(par[1] == 6.0); BOOST_TEST(par[3] == 6.0);
    BOOST_TEST(par[2] == 2.0); BOOST_TEST(par[64] == 6.0); BOOST_TEST(par[197] == 6.0);
    BOOST_TEST(par[100] == 2.0);

    std::vector<double> pp(256, 1.5), ph(256, 0.5);
    const double a_par = monoprop::detail::accumulate_cos_words(pp.data(), ph.data(), cos, 0.7, 0.9);
...
BOOST_AUTO_TEST_CASE(packed_cross_rank_storage_bit_packs_binary_phases) {
    ...
    for (size_t idx = 0; idx < 128; ++idx) {
        binary_cross_rank[1].b_indices.push_back(idx + 1005);
    }
    for (size_t idx = 0; idx < 128; ++idx) {
        binary_cross_rank[1].b_indices.push_back(idx + 5);
    }
    // Single phased D list: D- (out) block first (stored -phase), then D+ (in) block (stored +(-phase)).
    for (size_t idx = 0; idx < 128; ++idx) {
        const int phase = idx % 2 == 0 ? 1 : -1;
        binary_cross_rank[1].d.push_back({idx + 5, -phase});
    }
    for (size_t idx = 0; idx < 128; ++idx) {
        const int phase = idx % 2 == 0 ? 1 : -1;
        binary_cross_rank[1].d.push_back({idx + 1005, -phase});
    }
    ...
    wide_phase_cross_rank[1].d[0].second = -2;
    ...
    BOOST_CHECK(binary_storage.d_phases.uses_binary_phases);
    // Non-binary phase (2) forces full int8 storage.
    BOOST_CHECK(!wide_phase_storage.d_phases.uses_binary_phases);

    // D^-[1] = {idx+5=6, phase=-1}; stored as -(-1) = 1.
    BOOST_CHECK_EQUAL(detail::cross_rank_d_phase(binary_storage, 1, 1), 1);
    // D^+[0] = {idx+1005=1005, -phase=-1}; stored as +(-1) = -1. (D^+ at flat index 128)
    BOOST_CHECK_EQUAL(detail::cross_rank_d_phase(binary_storage, 1, 128), -1);
