...
// persistent even-parity sidecar truncated to each layer's cos_count.
template <size_t NumModes>
auto fold_cos(const EvenParityMajoranaScanSidecar<NumModes> &sidecar, const Layer &layer) -> CosineWordList {
    MajoranaSet<NumModes> gen{};
    const auto &gw = layer.generator_words();
    std::memcpy(gen.data(), gw.data(), gw.size() * sizeof(uint64_t));
    const auto fold = monoprop::detail::make_prepared_fold<NumModes>(sidecar, gen, layer.cos_count());
    return monoprop::detail::fold_to_cos_words<NumModes>(fold);
}
...
BOOST_AUTO_TEST_CASE(pare_graph_emits_expected_layer_kinds) {
    ...
    const auto &sidecar = sim.mp_op().even_parity_scan_sidecar();
    ...
    // Provider: real fold cos for layer 0 PLUS the synthetic index; real fold cos for all others.
    auto provider = [&](size_t i) -> CosineWordList {
        CosineWordList cos = fold_cos<kNumModes>(sidecar, graph.get_layer(i));
        if (i == marked_layer) {
            bool merged = false;
            for (auto &b : cos.blocks) {
                if (b.first == synth_base) { b.second |= synth_bit; merged = true; break; }
            }
            if (!merged) { cos.blocks.emplace_back(synth_base, synth_bit); }
            ++cos.total_count;
        }
        return cos;
    };
    ...
    for (size_t i = 0; i < pared.layers(); ++i) {
        ...
        if (std::holds_alternative<PrunedLayer>(layer.kind)) {
            ++pruned_count;
            const CosineWordList *pruned = layer.pruned_cos();
            ...
            // Preserved layers are FoldLayer (cos recomputed at replay, nothing stored).
            BOOST_TEST(std::holds_alternative<FoldLayer>(layer.kind));
            BOOST_TEST(layer.pruned_cos() == static_cast<const CosineWordList *>(nullptr));
        }
    }
    // Exactly the marked layer should be pruned (its synthetic index dropped).
    BOOST_TEST(pruned_count >= 1u);
    BOOST_TEST(std::holds_alternative<PrunedLayer>(pared.get_layer(marked_layer).kind));
}
...
BOOST_AUTO_TEST_CASE(pare_graph_energy_matches_unpared) {
    ...
    sim_full.propagate(data.majoranas);
    auto ev_full = sim_full.expectation_value_functional(data.param_inds, data.gen_coeffs, std::nullopt);
    ...
    sim_tiny.propagate(data.majoranas);
    auto ev_tiny = sim_tiny.expectation_value_functional(data.param_inds, data.gen_coeffs, std::optional<double>{1e-12});
    ...
    sim_real.propagate(data.majoranas);
    auto ev_real = sim_real.expectation_value_functional(data.param_inds, data.gen_coeffs, std::optional<double>{1e-10});
