...
bench-ci-rungs LABEL:
    ...
    while IFS='|' read -r head args <&3; do
      ...
      # a non-numeric one reaches the arithmetic below as an unset name, so worth 0; `0`
      # divides by zero at per-rank and reaches `mpiexec -n 0` otherwise; and `08` is an
      # invalid octal literal there. Rejecting leading zeros keeps that arithmetic base 10.
      for field in ranks partitions rounds; do
        ...
        case "$value" in
          ...
          *[!0-9]* | 0*)
            echo "::error::rung $rung needs a positive decimal $field, not '$value'"; exit 1 ;;
        esac
      done
