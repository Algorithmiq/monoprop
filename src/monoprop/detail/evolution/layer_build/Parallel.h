...
// (PareGraph.cpp::filter_layer_cosine_data open-codes this same chunk-and-concat pattern with a raw
// tbb::parallel_for because it also AND-reduces a per-chunk `preserves` flag alongside the concat.)
...
inline auto for_each_chunk(size_t n, size_t chunks, Body &&body) -> void {
    ...
    tbb::parallel_for(
        ...
        [&, prof_r](const tbb::blocked_range<size_t> &br) {
            profiling::TaskScope prof_ts(prof_r);
