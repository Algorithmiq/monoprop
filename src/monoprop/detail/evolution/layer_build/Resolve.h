...
// ─── Shared incoming-query probe (Phases 1-2 + insert) ────────────────────────
// resolve_incoming_queries and its fused twin share the picture-independent probe/insert machinery:
// deserialize every incoming query, batch-find it in the local operator, and assign each miss the next
// index base+j in a serial (sender,query)-order prefix. None of this does floating-point math, so both
// resolvers reuse it verbatim — the deterministic base+j assignment (and thus multi-rank bit-exactness)
// then CANNOT drift between the twins. Each resolver supplies its own Phase-3 scatter (the only part
// that differs: graph acc entries + TermIndex responses vs. half-rotation records + value responses)
// BETWEEN probe_incoming_queries and insert_incoming_misses.
//
...
// Phases 1-2: deserialize + batch-find every incoming query and assign miss indices. Read-only w.r.t.
// the operator's contents (probes only); the caller runs its Phase-3 scatter, then insert_incoming_misses.
template <size_t NumModes>
...
auto resolve_incoming_queries_fused(const std::vector<VecZ> &incoming,
                                    const std::vector<std::vector<double>> &inc_val,
                                    MPOperator<NumModes> &op,
                                    size_t rank_count,
                                    bool is_leader_pass,
                                    MatchedEpochSet &matched,
