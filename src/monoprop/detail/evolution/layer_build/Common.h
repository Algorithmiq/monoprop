...
// Fused query+value record width (ContractImmediately R>1): the plain query record plus ONE trailing
// word holding the source's pre-cos coefficient (v_src), bit-cast from double. This lets the query and
// value streams ride a SINGLE alltoallv instead of two, cutting a full count+payload round off every
// gate (see LayerBuildEngine::run_exchange). VecZ's element is size_t (64-bit), matching double, so the
// bit-cast round-trip is exact ⇒ the fused exchange is byte-identical to the two-stream exchange.
template <size_t NumModes>
...
// The maj words and phase word occupy the SAME leading offsets in both the plain (kQueryWords) and the
...
// Interleave a rank's plain query records (`q`, kQueryWords each) with its parallel value stream (`v`,
// one double per query) into the fused send buffer `out` (kQueryWordsFused each). `out` is reused across
// gates (clear + capacity-preserving reserve = high-water-mark, no per-gate shrink). Requires
// v.size() == q.size()/kQueryWords (the two streams are built element-for-element aligned at scan emit).
template <size_t NumModes>
