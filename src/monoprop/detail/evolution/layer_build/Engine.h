...
struct LayerBuildEngine {
    ...
    auto run_exchange(bool is_leader_pass) -> void {
        ...
        if (fused_ != nullptr) {
            ...
            // Round 1: queries A→B, with the v_src VALUE stream OVERLAPPED (a second alltoallv posted
            // before either completes). The query COUNT Alltoall is done when begin_alltoallv returns, so
            // received_counts() (in WORDS) is valid immediately — divide by kQueryWords for the value
            // stream's known per-source recv counts (skips its count Alltoall).
            auto qh = mpi::begin_alltoallv(queries_r, comm);
            std::vector<int> val_recv_counts(R);
            for (size_t r = 0; r < R; ++r) {
                val_recv_counts[r] = qh.received_counts()[r] / static_cast<int>(kQueryWords<NumModes>);
            }
            auto vh = mpi::begin_alltoallv(src_val_r, comm, /*skip_self=*/false, &val_recv_counts);
