...
//     phases run sequentially on the main thread inside propagate(), so a region's
...
//     as a TaskScope, so pool worker threads — which do not inherit the dispatcher's thread-local —
...
// single shared state, busy-time whose dispatch marker and threading primitive land in different modules
