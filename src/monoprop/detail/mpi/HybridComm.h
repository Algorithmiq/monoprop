    ...
    auto alltoallv_reverse_impl_(int local_partition,
        ...
        if (local_partition == 0) {
            ...
            for (int b = 0; b < r_; ++b) {
                ...
                rev_send_displs_[i] = checked_int_(send_total);
                rev_recv_displs_[i] = checked_int_(recv_total);
...
private:
    ...
    auto size_staging_parallel_(int local_partition, size_t elem, RecvCountFn recv_count) -> void {
        ...
        if (local_partition == 0) {
            ...
            grow_(stage_send_, static_cast<size_t>(checked_int_(send_running)) * elem);
            grow_(stage_recv_, static_cast<size_t>(checked_int_(recv_running)) * elem);
        }
