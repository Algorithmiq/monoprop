...
namespace {
// One entry per shipped variant, best first: its id, and whether this CPU can execute it. Both
// answers come off the same generated table so they cannot drift apart.
auto variant_support() -> std::vector<std::pair<std::string, bool>> {
    ...
    __builtin_cpu_init();
#define monoprop_ADD_VARIANT(id, predicate) out.emplace_back(id, static_cast<bool>(predicate));
...
auto supported_variants() -> std::vector<std::string> {
    ...
    for (const auto &[id, supported] : variant_support()) {
        if (supported) {
            out.push_back(id);
        }
    }
...
auto known_variants() -> std::vector<std::string> {
    auto out = std::vector<std::string>{};
    for (const auto &entry : variant_support()) {
...
NB_MODULE(_isa, mod) {
    ...
    mod.def("supported_variants", &supported_variants, "ISA variants the running CPU can execute, best first.");
