...
// One class, no width in its name: MonomialPropagator no longer has a mode-count template parameter,
// so there is exactly one C++ type to register and nothing left for a generator to enumerate.
inline auto bind_monomial_propagator(nb::module_ &mod) -> void {
    ...
    cls.def_prop_rw(
        "cutoff_type",
        [](const MonomialPropagator &self) -> std::string { return cutoff_type_enum_2_str(self.cutoff_type()); },
    ...
    cls.def_prop_ro(
        "basis",
        [](const MonomialPropagator &self) -> std::string { return basis_enum_2_str(self.basis()); },
