...
described in terms of generic **terms** so it stays agnostic to the underlying
fermionic/qubit family.

Gate information (the term generators, their coefficients, and the parameter each drives)
...
class MonomialPropagator(ABC):
    ...
    def _validate_and_correct_only_rotate_len_k(
        ...
        simulator. The simulator interprets ``0`` as "apply all gates to all terms",
    ...
    def size(self) -> int:
        ...
        Returns:
            The number of distinct terms in the simulator's current representation.
        """
