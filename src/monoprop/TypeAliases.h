...
// position list directly. Used by the even-parity sidecar, the heaviest per-row op reader.
...
// Hash a single-word MajoranaSet directly from its raw word. Returns 0 (unused) for multi-word
// NumModes; callers must only invoke it on the single-word fast path.
template <size_t NumModes>
inline auto single_word_majorana_hash(uint64_t word) noexcept -> size_t {
    if constexpr (MajoranaSet<NumModes>::num_words() == 1) {
        return static_cast<size_t>(SplitmixHash<MajoranaSet<NumModes>>::mix(word));
    }
    else {
        return 0;
    }
...
// Operator row index. Default uint32_t (memory-minimal). Define MONOPROP_WIDE_TERM_INDEX to support
// > 2^32 local terms on a single rank, at the cost of doubling the per-term index arrays.
...
// MUST be declared BEFORE Sidecar.h / MPOperator.h: those headers' templates call these accessors,
