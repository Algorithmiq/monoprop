...
inline auto cross_rank_b_size(const Layer &layer, size_t rank) -> size_t { return layer.cross_rank_b_size(rank); }
inline auto cross_rank_d_size(const Layer &layer, size_t rank) -> size_t { return layer.cross_rank_d_size(rank); }
...
auto has_remote_cross_rank_edges(const Layer &layer, size_t my_rank) -> bool {
    ...
    for_each_remote_rank(layer, my_rank, [&](size_t rank) {
        if (cross_rank_b_size(layer, rank) != 0 || cross_rank_d_size(layer, rank) != 0) {
...
auto build_builder_exchange_layout(const Layer &layer, size_t my_rank, BuilderExchangeDirection direction)
    ...
    for_each_remote_rank(layer, my_rank, [&](size_t rank) {
        ...
        const size_t send_count = direction == BuilderExchangeDirection::Outgoing
                                      ? cross_rank_b_size(layer, rank)
                                      : cross_rank_d_size(layer, rank);
...
auto pack_source_keep_flags(const Layer &layer,
    ...
    for_each_remote_rank(layer, my_rank, [&](size_t rank) {
        ...
        const size_t count = cross_rank_b_size(layer, rank);
        layer.for_each_cross_rank_b_range(rank, 0, count, [&](size_t logical_idx, size_t src_idx) {
...
auto execute_builder_exchange(const BuilderExchangeLayout &layout, BuilderExchangeBuffers &buffers, MPI_Comm comm)
    -> void {
    // All ranks in the communicator must call MPI_Alltoallv together.
    // Do not skip even when total_send + total_recv == 0: skipping on one rank
    // while another has non-zero counts causes a deadlock. Buffers are always
    // sized >= 1 (see resize_builder_exchange_buffers).
...
// ── Cosine filter (identical semantics to LayerFiltering.cpp::filter_layer_cosine_data) ──
...
inline auto keep_mask_for_block(const std::vector<char> &keep, size_t base, uint64_t present) -> uint64_t {
    ...
    while (b) {
        ...
        if (idx < keep.size() && keep[idx] != 0) { mask |= (uint64_t{1} << t); }
        b &= b - 1;
    }
    return mask;
}

auto filter_layer_cosine_data(const CosineWordList &cos, const std::vector<char> &nodes_to_keep)
    -> std::pair<CosineWordList, bool> {
    ...
    auto scan = [&](size_t begin, size_t end, CosineWordList &out, bool &preserves) {
        for (size_t k = begin; k < end; ++k) {
            const auto [base, bits] = cos.blocks[k];
            const uint64_t kept = bits & keep_mask_for_block(nodes_to_keep, base, bits);
            if (kept != bits) { preserves = false; }
            if (kept) { out.blocks.emplace_back(base, kept); out.total_count += static_cast<size_t>(std::popcount(kept)); }
        }
    };

    if (!use_parallel) {
        CosineWordList filtered;
        bool preserves = true;
    ...
    struct Part { CosineWordList list; bool preserves = true; };
...
auto mark_replayed_d_targets(const Layer &layer, std::vector<char> &nodes_to_keep) -> void {
    ...
    for (size_t rank = 0; rank < rank_count; ++rank) {
        layer.for_each_cross_rank_d_range(rank, 0, cross_rank_d_size(layer, rank),
...
auto propagate_cross_rank_b(const Layer &layer,
    ...
    for_each_remote_rank(layer, my_rank, [&](size_t rank) {
        ...
        layer.for_each_cross_rank_b_range(rank, 0, cross_rank_b_size(layer, rank),
                                          [&](size_t logical_idx, size_t src_idx) {
            const bool selected = base + logical_idx < selection_recv.size() && selection_recv[base + logical_idx] != 0;
...
auto pare_graph(const MPGraph &graph,
                ...
                bool schrodinger,
                MPI_Comm comm,
                const std::function<CosineWordList(size_t)> &full_cos_of_layer) -> MPGraph {
    ...
    for (size_t iter = 0; iter < num_layers; ++iter) {
        ...
        const CosineWordList full = full_cos_of_layer(layer_idx);
        ...
        layers[layer_idx] = preserves
                                ? Layer(layer.shared_core())
                                : Layer(LayerKind{PrunedLayer{layer.shared_core(), std::move(filtered)}});
    }
